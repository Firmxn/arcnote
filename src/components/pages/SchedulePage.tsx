...
export const SchedulePage: React.FC<SchedulePageProps> = ({ initialEventId }) => {
    ...
    return (
        <div className="flex-1 h-screen flex flex-col bg-neutral text-text-neutral relative overflow-hidden">
            {/* Header Toolbar */}
            <div className="flex items-center justify-between px-8 py-4 border-b border-secondary/20 bg-neutral/95 backdrop-blur-sm sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <Button
                            variant="ghost"
                            className="flex items-center gap-2 text-2xl font-bold cursor-pointer hover:text-primary hover:dark:text-accent transition-colors group px-0 h-auto hover:bg-transparent"
            ...
            <div className="flex-1 p-8 overflow-y-auto">
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-7 gap-px bg-secondary/20 border border-secondary/20 rounded-lg overflow-hidden min-h-[600px]">
                        {/* Headers */}
                        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d) => (
                            <div key={d} className="bg-neutral p-3 text-center text-sm font-semibold text-text-neutral/70 border-b border-secondary/10">
                                {d}
                            </div>
                        ))}
                        ...
                        {calendarDays.map((dateItem) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`
                                    min-h-[100px] p-2 cursor-pointer transition-colors flex flex-col relative
